...
function main() {    
    var selectedLayers = app.selection;
    for(var i = 0; i < selectedLayers.length; i++) {
        var textFrame = selectedLayers[i] instanceof InsertionPoint ? selectedLayers[i].parentTextFrames[0] : selectedLayers[i];
        if(textFrame instanceof TextFrame) {
            textFrame.textFramePreferences.autoSizingReferencePoint = AutoSizingReferenceEnum.CENTER_POINT;
            textFrame.textFramePreferences.autoSizingType = AutoSizingTypeEnum.HEIGHT_AND_WIDTH;
            var text = textFrame.contents.replace(/[^\S\n\r]+$/gmi, ' ');
            var toArray = text.split(/\s/);
            textFrame.contents = shaper(textFrame, toArray);
        }
    }
}
